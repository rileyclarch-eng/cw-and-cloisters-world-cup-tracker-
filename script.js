...
function getTeamScore(teamName) {
  const team = teamInfo(teamName);
  if (!team || team.status === "eliminated") return 0;
  return team.stage;
}
...
function getPlayerProfile(player) {
  ...
  const aliveCount = player.teams.filter((team) => teamInfo(team).status !== "eliminated").length;
  const dangerCount = player.teams.filter((team) => teamInfo(team).status === "danger").length;
...
function buildDailyUpdateText(profiles) {
  const leader = profiles[0];
  const cooked = getMostCooked(profiles);
  const dangerMatch = getNextDangerMatch();
...
function setupCopyUpdate(profiles) {
  const button = el("#copyUpdateButton");
  const status = el("#copyStatus");
  if (!button || !status) return;

  button.addEventListener("click", async () => {
    const updateText = buildDailyUpdateText(profiles);
    try {
...
function finishLoader() {
  ...
  const delay = reduceMotion ? 120 : hasLoadedThisSession ? 80 : 1050;
...
function initDashboard() {
  ...
  const profiles = sortProfiles(sweepstakeData.players.map(getPlayerProfile));
  renderLeader(profiles[0]);
  renderOverview(profiles);
  renderLeaderboard(profiles);
  renderInsights(profiles);
  renderPlayers(profiles);
  renderLists();
  setupCopyUpdate(profiles);
